...
@AUTHOR - Nicolas Angelard-Gontier - 260532513
"""
...
PATH_TO_OUTPUT = "./data/ml_dataset_test_out.csv"
...
def getImportantWords(text):
    ...
    text = map(lambda w: ps.stem(w), word_tokenize(text)) # apply stemming to the tokenized text.
    return filter(lambda w: w not in EXCLUDE, text) # exculde some common words from the array of tokens.
...
@param neighbor_word_proba - the probability of each word for this neighbor. ex: {'word1':<proba1>, 'word2':<proba2>, ...}
...
def distance(item, neighbor_id, neighbor_word_proba):
    ...
    item_word_proba = {} # probability of each word in this item. ex: {'word1':<proba1>, 'word2':<proba2>, ...}
...
example of neighbors_word_proba: {
    "id1": {'word1':<proba1>, 'word2':<proba2>, ...},
    "id2": {'word3':<proba3>, ...},
    ...
}
@return the predicted class of the item as an integer (0,1,2,or 3).
"""
...
#writeData(predictions, PATH_TO_OUTPUT)
#print "file created."
